...
The following PROD requirements are mapped to Phase 18 in REQUIREMENTS.md but are NOT in the ROADMAP success criteria. They are deferred to v6:

1. **PROD-21 (Centralize table name constants)** - Medium priority
   - Reason: Current scale does not require centralization. Table names are consistent in code.
   - Impact: Low. No runtime issues, purely code organization.
   - v6 consideration: Implement if multi-database support is added.

2. **PROD-22 (Add cache TTL to config/agent/service caches)** - Already addressed
   - Reason: Phase 16 added `@lru_cache(maxsize=100)` to routes.py DI caching.
   - Impact: None. The caching concern has been mitigated.
   - Note: lru_cache has implicit eviction via maxsize. Explicit TTL would require additional dependency.

3. **PROD-23 (Optimize MMR search O(n^2) complexity)** - Medium priority
   - Reason: Current FAISS index size (~1000 documents) does not cause performance issues.
   - Impact: Low at current scale. MMR search completes in <100ms.
   - v6 consideration: Implement if document count exceeds 10,000 or if latency exceeds 500ms.
</deferred_requirements>
