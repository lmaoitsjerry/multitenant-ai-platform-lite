...
must_haves:
  truths:
    ...
    - "privacy_routes.py array accesses at lines 406, 461, 474, 629, 770 have bounds checking"
    - "notifications_routes.py array accesses at lines 343, 553 have bounds checking"
    - "routes.py array accesses at lines 1181, 1299 have bounds checking"
    - "analytics_routes.py array accesses at lines 912, 1070 have bounds checking"
  artifacts:
...
<task type="auto">
  ...
  1. In privacy_routes.py:
     - Line 406: Change `dsar_response.data[0]["id"]` to:
       `dsar_response.data[0]["id"] if dsar_response.data else None`
     - Line 461: Same pattern for array access
     - Line 474: This is inside DSAR deletion logic. Wrap `background_tasks.add_task()` call in conditional:
       ```python
       if dsar_response.data:
           background_tasks.add_task(...)
       ```
       This prevents scheduling background task when there's no DSAR record to process.
     ...
     - Line 770: Ensure `client.data[0]` has bounds check:
       `client.data[0] if client.data else None`

  2. In notifications_routes.py (lines 343, 553):
     - Line 343: Already has pattern `result.data[0] if result.data else update_data` - verify present
     - Line 553: Already has pattern `result.data[0]['id'] if result.data else None` - verify present
  ...
  4. In analytics_routes.py (lines 912, 1070):
     - Already have `rows[0] if rows else None` pattern - verify present
  ...
  Focus on fixing any remaining unsafe patterns, especially in privacy_routes.py.
  </action>
  <verify>
  ...
  - privacy_routes.py line 474 background_tasks.add_task wrapped in data check
