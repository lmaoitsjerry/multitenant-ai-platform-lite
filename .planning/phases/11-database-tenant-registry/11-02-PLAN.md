...
must_haves:
  truths:
    - "TenantConfigService loads config from database when available"
    - "TenantConfigService falls back to YAML when database record missing"
...
Output: New TenantConfigService class, updated config/loader.py to use it
</objective>
...
class TenantConfigService:
    ...
    # Test tenants that remain as YAML files (not migrated)
    YAML_ONLY_TENANTS = {
        'africastay',
        'safariexplore-kvph',
        'safarirun-t0vc',
    ...
    def get_config(self, tenant_id: str) -> Optional[Dict[str, Any]]:
        """
        Get tenant configuration (database first, YAML fallback)
        ...
        """
        # Check if this is a YAML-only tenant
...
- YAML_ONLY_TENANTS contains: africastay, safariexplore-kvph, safarirun-t0vc, beachresorts, example
  </verify>
  <done>
