...
must_haves:
  truths:
    ...
    - "CI workflow fails if coverage drops below 70%"
  artifacts:
    - path: "pyproject.toml"
...
<task type="auto">
  <name>Task 4: Integrate coverage reporting in CI workflow</name>
  <files>.github/workflows/ci.yml</files>
  <action>
Update the CI workflow to run coverage checks and fail if coverage < 70%.

Replace the "Run tests" step in ci.yml with:
...
Key changes:
- Adds --cov flags to track coverage
- --cov-fail-under=70 ensures CI fails if coverage drops below 70%
- --cov-report=term-missing shows which lines aren't covered
  </action>
  <verify>
```bash
# Verify ci.yml has coverage flags
grep "cov-fail-under" .github/workflows/ci.yml
```
  </verify>
  <done>
CI workflow updated to run coverage checks with fail_under=70, ensuring coverage requirement is enforced automatically.
  </done>
</task>
