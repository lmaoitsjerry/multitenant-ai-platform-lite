...
class TestDICachingThreadSafety:
    """Tests for thread-safe DI caching in dependencies.py"""

    def test_cache_returns_same_config(self):
        """Verify cache returns same instance for same client_id"""
        from src.api.dependencies import get_client_config, _client_configs

        _client_configs.clear()

        with patch('src.api.dependencies.ClientConfig') as mock_config:
            mock_config.return_value = MagicMock(client_id='test-tenant')

            config1 = get_client_config(x_client_id='test-tenant')
            config2 = get_client_config(x_client_id='test-tenant')
    ...
    def test_concurrent_config_access(self):
        ...
        with patch('src.api.dependencies.ClientConfig') as mock_config:
            ...
            def access_config():
                try:
                    config = get_client_config(x_client_id='concurrent-tenant')
...
class TestCacheThreadSafety:
    ...
    def test_lru_cache_is_thread_safe(self):
        ...
        # Should have been called only once (or at most once per thread due to race)
        assert call_count <= 20  # 20 threads means worst-case each calls before cache populates
    ...
    def test_different_keys_cached_independently(self):
        ...
        with patch('src.api.dependencies.ClientConfig') as mock_config:
            ...
            config_a = get_client_config(x_client_id='tenant-a')
            config_b = get_client_config(x_client_id='tenant-b')
