...
class TestInboundRoutesDependencies:
    """Test dependency injection for inbound routes."""

    def test_get_client_config_with_invalid_client(self, mock_config):
        """get_client_config should handle invalid client IDs."""
        from src.api.inbound_routes import get_client_config, _client_configs
        ...
        with patch('src.api.inbound_routes.ClientConfig') as mock_class:
    ...
    def test_get_client_config_uses_env_fallback(self, mock_config):
        """get_client_config uses CLIENT_ID env var when header is None."""
        from src.api.inbound_routes import get_client_config, _client_configs
        import os
        ...
        with patch('src.api.inbound_routes.ClientConfig') as mock_class:
            ...
            with patch.dict(os.environ, {"CLIENT_ID": "env_client"}):
                ...
                mock_class.assert_called_with("env_client")

    def test_get_client_config_defaults_to_example(self, mock_config):
        """get_client_config defaults to 'example' when no header or env."""
        from src.api.inbound_routes import get_client_config, _client_configs
        import os
        ...
        with patch('src.api.inbound_routes.ClientConfig') as mock_class:
            mock_class.return_value = mock_config
            # Remove CLIENT_ID from environment
            env = os.environ.copy()
            if "CLIENT_ID" in env:
                del env["CLIENT_ID"]
            with patch.dict(os.environ, env, clear=True):
                config = get_client_config(None)
                # Should default to "example"
